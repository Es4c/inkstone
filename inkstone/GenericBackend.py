...
#        if type(i) is self.raw_type or type(o) is self.raw_type:
 #           return i
    ...
    def concat(self, tup, dim=0):
        match self.backend:
            case "torch":
                return torch.concat(tup,dim=dim)
            case "numpy":
                return np.concatenate(tup, axis=dim)
            case _:
                raise NotImplementedError
    ...
    def argsort(self, a, b=-1, c=None, d=None):
        ...
        match self.backend:
            case "torch":
                ...
                return torch.argsort(a,dim=b,descending=c,stable=d)
            case "numpy":
                return np.argsort(a,b,kind=c,order=d)
            case _:
                raise NotImplementedError
